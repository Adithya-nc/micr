...
# ── database/engine.py ─────────────────────────────────────────────────────
w("backend/app/database/engine.py", r"""
import sqlite3, json
from pathlib import Path
...
CREATE TABLE IF NOT EXISTS environmental_fields (
    ...
    field_type TEXT NOT NULL, timestamp TEXT NOT NULL,
    valid_time_start TEXT, valid_time_end TEXT, source TEXT NOT NULL,
    source_version TEXT, resolution_deg REAL, region_geojson TEXT,
...
CREATE TABLE IF NOT EXISTS particle_trajectories (
    id TEXT PRIMARY KEY, spill_id TEXT REFERENCES oil_spills(id),
    run_type TEXT NOT NULL, windage_coefficient REAL NOT NULL,
    particle_count INTEGER NOT NULL, integration_timestep_min INTEGER NOT NULL,
    integration_hours REAL NOT NULL, integration_method TEXT NOT NULL,
    particles_json TEXT NOT NULL, origin_region_geojson TEXT,
    origin_centroid_geojson TEXT, origin_time_estimate TEXT,
    origin_time_uncertainty_h REAL, spatial_uncertainty_km REAL,
    environmental_source TEXT, data_mode TEXT NOT NULL, provenance TEXT NOT NULL, created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS attributions (
    ...
    norm_proximity REAL, norm_temporal REAL, norm_trajectory REAL,
    norm_heading REAL, norm_continuity REAL,
    weight_proximity REAL, weight_temporal REAL, weight_trajectory REAL,
    weight_heading REAL, weight_continuity REAL,
    evidence_score REAL, data_confidence REAL, final_score REAL,
    behaviour_observations_json TEXT,
    ais_gap_detected INTEGER DEFAULT 0, ais_gap_duration_min REAL,
...
CREATE INDEX IF NOT EXISTS idx_ais_obs_mmsi ON ais_observations(mmsi, timestamp);
CREATE INDEX IF NOT EXISTS idx_attributions_spill ON attributions(spill_id);
CREATE INDEX IF NOT EXISTS idx_particle_traj_spill ON particle_trajectories(spill_id, run_type);
"""
