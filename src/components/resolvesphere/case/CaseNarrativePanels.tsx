  ...
  return (
    <section className="rounded-lg border border-destructive/50 bg-destructive/5 p-4">
      ...
      <dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2">
        {sources.map((source) => (
          <div key={source.system}>
            <dt className="text-xs text-muted-foreground">{source.system}</dt>
            <dd>{source.value}</dd>
          </div>
        ))}
      </dl>
  ...
  return (
    <section className="rounded-lg border border-warning/50 bg-warning/5 p-4">
      ...
      <p className="mt-2 text-sm">
        Account records were checked first. One detail is still needed:
      </p>
  ...
  return (
    <section className="rounded-lg border bg-card">
      ...
      <dl className="grid gap-3 px-4 py-4 text-sm sm:grid-cols-2">
        <div>
          <dt className="text-xs text-muted-foreground">Reported problem</dt>
          <dd>{String(passport.reported_problem ?? '—')}</dd>
        </div>

        <div>
          <dt className="text-xs text-muted-foreground">Actual problem</dt>
          <dd>{String(passport.actual_problem ?? '—').replace(/_/g, ' ')}</dd>
        </div>

        <div>
          <dt className="text-xs text-muted-foreground">Policy</dt>
          <dd>{String(passport.policy_id ?? '—')}</dd>
        </div>
        ...
        <div>
          ...
          <dd>
            <StatusBadge
              value={String(passport.final_state ?? 'UNKNOWN')}
              tone="success"
            />
          </dd>
        </div>
...
export function CaseBriefPanel({
  ...
  const reasons: Record<string, string> = {
    MATERIAL_CONTRADICTION:
      'Order and billing systems disagree on the refund state, so the true position is unconfirmed.',
    CUSTOMER_STATEMENT_INSUFFICIENT:
      'Only the customer statement supports the claim; policy requires a system record before a refund.',
    MANUAL_REVIEW_REQUIRED:
      'Available records do not authorise an automatic resolution.',
  }

  return (
    <section className="rounded-lg border bg-card">
      <header className="border-b px-4 py-3">
        <h2 className="text-sm font-semibold">Case brief for the specialist</h2>
      </header>

      <dl className="divide-y text-sm">
        <div className="px-4 py-3">
          <dt className="text-xs text-muted-foreground">Customer issue</dt>
          <dd className="mt-1">{caseRow.raw_complaint}</dd>
        </div>

        <div className="px-4 py-3">
          <dt className="text-xs text-muted-foreground">Root cause analysis</dt>
          <dd className="mt-1">{caseRow.root_cause ?? 'Not determined.'}</dd>
        </div>

        <div className="px-4 py-3">
          <dt className="text-xs text-muted-foreground">Why it was escalated</dt>
          <dd className="mt-1">
            {reasons[caseRow.escalation_reason ?? ''] ??
              caseRow.escalation_reason ??
              'Not recorded'}
            {caseRow.escalation_reason && (
        ...
        <div className="px-4 py-3">
          <dt className="text-xs text-muted-foreground">Evidence gathered</dt>
          <dd className="mt-1">
            {evidenceCount} records reviewed
            {decision
              ? `, policy ${decision.policy_id}, risk score ${decision.risk_score}`
              : ''}
          </dd>
        </div>

        <div className="px-4 py-3">
          <dt className="text-xs text-muted-foreground">Recommended next step</dt>
          <dd className="mt-1">
            Confirm the disputed record with the source system, then approve or
