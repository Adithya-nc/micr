...
export function AgentActivity({
  ...
  const handoffs = events.filter(
    (event) => event.event_type === 'AGENT_HANDOFF',
  )
          ...
          {handoffs.map((event) => (
            <li key={event.event_id} className="px-4 py-3 text-sm">
              <p className="flex flex-wrap items-center gap-2 font-medium">
                {String(event.payload.from ?? 'Unknown')}
                <ArrowRight className="h-3.5 w-3.5 text-muted-foreground" />
                {String(event.payload.to ?? 'Unknown')}
              </p>
...
export function RoutingPanel({
  ...
  const rows = [
    {
      label: 'Intent',
      value: caseRow.primary_intent?.replace(/_/g, ' '),
    },
    ...
    <section className="rounded-lg border bg-card">
      ...
      <dl className="divide-y">
        <div className="flex items-center justify-between gap-4 px-4 py-2 text-sm">
          ...
          <dd className="text-right">
            {outcome[decision.auth_state] ?? decision.auth_state}
          </dd>
        </div>

        <div className="flex items-center justify-between gap-4 px-4 py-2 text-sm">
          <dt className="text-muted-foreground">Policy applied</dt>
          <dd>{decision.policy_id}</dd>
        </div>
