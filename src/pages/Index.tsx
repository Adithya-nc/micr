...
import { ArrowRight, CheckCircle2, ChevronRight, Clock3, FileCheck2, MessageSquare, Radar, ShieldCheck, Sparkles, Workflow, Zap } from 'lucide-react';
...
const scenarios = [{
  ...
  number: '01',
  title: 'Investigate',
  description: 'The system gathers order, payment, customer and policy evidence before making a decision.'
}, {
  icon: ShieldCheck,
  number: '02',
  title: 'Policy-gate',
  description: 'Deterministic controls validate evidence quality, risk, freshness and contradictions.'
}, {
  icon: Zap,
  number: '03',
  title: 'Act safely',
  description: 'Approved actions are executed with traceability, idempotency and controlled escalation.'
}, {
  icon: FileCheck2,
  number: '04',
  title: 'Verify',
  description: 'The system checks the authoritative post-action state before declaring resolution.'
}];
const metrics = [['01', 'Evidence-first decisions'], ['02', 'Policy-controlled actions'], ['03', 'Human escalation when needed'], ['04', 'Verified outcomes']];
export default function Index() {
  return <div className="min-h-screen overflow-hidden bg-[#f8fafc] text-slate-950 dark:bg-slate-950 dark:text-white">
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-white/85 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/85">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 lg:px-8">
          ...
          <nav className="hidden items-center gap-7 md:flex">
            <a href="#how" className="text-sm font-medium text-slate-500 transition hover:text-slate-950 dark:hover:text-white">
              How it works
            </a>

            <a href="#capabilities" className="text-sm font-medium text-slate-500 transition hover:text-slate-950 dark:hover:text-white">
              Capabilities
            </a>

            <Link to="/chat" className="text-sm font-medium text-slate-500 transition hover:text-slate-950 dark:hover:text-white">
              Customer portal
            </Link>
          </nav>
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to="/login" className="hidden rounded-lg px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 sm:block dark:text-slate-300 dark:hover:bg-slate-900">
              Sign in
            </Link>

            <Button asChild className="rounded-lg bg-slate-950 px-4 shadow-lg shadow-slate-950/10 hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200">
              <Link to="/login">
                Open workspace
      ...
      <main>
          ...
          <div className="mx-auto grid max-w-7xl gap-14 px-5 pb-20 pt-16 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-8 lg:pb-28 lg:pt-24">
            ...
            <div>
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <Button asChild size="lg" className="h-12 rounded-xl bg-slate-950 px-6 text-sm font-bold shadow-xl shadow-slate-950/15 hover:bg-slate-800 dark:bg-white dark:text-slate-950">
                  <Link to="/login">
                ...
                <Button asChild variant="outline" size="lg" className="h-12 rounded-xl px-6 text-sm font-bold">
                  <Link to="/chat">
              ...
              <div className="mt-9 flex flex-wrap gap-x-6 gap-y-3">
                {['Evidence-first', 'Policy-controlled', 'Human-in-the-loop', 'Verified outcomes'].map(item => <div key={item} className="flex items-center gap-2 text-xs font-semibold text-slate-500">
            ...
            <div className="relative">
              ...
              <div className="overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-[0_30px_80px_-30px_rgba(15,23,42,.35)] dark:border-slate-800 dark:bg-slate-900">
                ...
                <div className="grid grid-cols-2 gap-3 p-4">
                  {[['Open cases', '24'], ['Investigating', '08'], ['Needs human', '03'], ['Resolved', '71']].map(([label, value]) => <div key={label} className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950">
                ...
                <div className="mx-4 mb-4 overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800">
                  ...
                  {[['PAY-7821', 'Resolved', 'Low'], ['ORD-4310', 'Investigating', 'Medium'], ['REF-1190', 'Escalated', 'High'], ['PAY-7814', 'Verified', 'Low']].map(([id, status, risk]) => <div key={id} className="grid grid-cols-[1.4fr_.8fr_.8fr] border-b border-slate-100 px-4 py-3 text-xs last:border-0 dark:border-slate-800">
        ...
        <section className="border-y border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
          <div className="mx-auto grid max-w-7xl grid-cols-2 divide-x divide-slate-200 sm:grid-cols-4 dark:divide-slate-800">
            {metrics.map(([number, label]) => <div key={number} className="flex items-center gap-3 px-5 py-5 lg:px-8">
        ...
        <section id="how" className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            return <div key={item.number} className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900">
                  <div className="flex items-center justify-between">
        ...
        {/* CAPABILITIES */}
        <section id="capabilities" className="border-y border-slate-200 bg-slate-950 text-white dark:border-slate-800">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 py-20 lg:grid-cols-2 lg:px-8 lg:py-28">
            <div>
              ...
              <Button asChild className="mt-8 rounded-xl bg-white text-slate-950 hover:bg-slate-200">
                <Link to="/login">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              const Icon = item.icon;
              return <div key={item.title} className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 transition hover:bg-white/[0.07]">
          ...
          <div className="relative overflow-hidden rounded-[2rem] bg-blue-600 px-7 py-12 text-white shadow-2xl shadow-blue-600/20 sm:px-12">
            ...
            <div className="relative max-w-3xl">
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                <Button asChild variant="outline" className="rounded-xl border-white/30 bg-transparent text-white hover:bg-white/10 hover:text-white">
                  <Link to="/chat">
                    Customer experience
                  </Link>
                </Button>
      ...
      <footer className="border-t border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-8 sm:flex-row sm:items-center sm:justify-between lg:px-8">
          ...
          <div className="flex gap-5 text-xs text-slate-500">
            <Link to="/terms" className="hover:text-slate-950 dark:hover:text-white">
              Terms
            </Link>

            <Link to="/privacy" className="hover:text-slate-950 dark:hover:text-white">
              Privacy
            </Link>
          </div>
        </div>
